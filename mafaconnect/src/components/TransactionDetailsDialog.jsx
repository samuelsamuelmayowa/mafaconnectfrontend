...
export function TransactionDetailsDialog({ open, onOpenChange, transaction }) {
  ...
  const sendReceiptEmail = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(
        `/api/transactions/${transaction.id}/send-receipt`,
        {
          method: "POST",
  ...
  // Prevent crashes if transaction is null during first render
  const safeTransaction = transaction || {};
  const status = safeTransaction.order_status || safeTransaction.status;

  const overdue = isOverdue(safeTransaction.due_date, status);
  const statusConfig = STATUS_CONFIG[status] || {
    label: status ?? "Unknown",
    variant: "secondary",
  };
  const typeConfig = TRANSACTION_TYPES[safeTransaction.transaction_type];

  // ⭐⭐⭐ REAL SUBTOTAL + TAX CALCULATION ADDED HERE
  const subtotal =
    safeTransaction.order_items?.reduce(
      (sum, item) => sum + (item.total_price || 0),
      0
    ) || 0;

  const tax =
    safeTransaction.tax_amount !== undefined
      ? safeTransaction.tax_amount
      : 0;

  const handleStatusChange = (newStatus) => {
    updateTransactionStatus({ id: safeTransaction.id, status: newStatus });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {safeTransaction.order_number ||
              `Transaction #${safeTransaction.id?.slice(0, 8)}`}
          </DialogTitle>
          ...
          {transaction && (
            <div className="flex gap-2 mt-2">
              ...
              <Badge
                variant={overdue ? "destructive" : statusConfig.variant}
...
// export function TransactionDetailsDialog({ open, onOpenChange, transaction }) {
//   // ✅ Hooks MUST run always — never inside conditions
//   const { updateTransactionStatus } = useTransactions();
//   const [isDownloading, setIsDownloading] = useState(false);
...
//   // Avoid read errors if transaction is not loaded yet
//   const safeTransaction = transaction || {};
//   const status = safeTransaction.order_status || safeTransaction.status;
...
//         {/* SHOW LOADING UI UNTIL TRANSACTION IS READY */}
//         {!transaction ? (
//           <div className="py-10 text-center text-muted-foreground">
//             Loading transaction details...
//           </div>
