...
export function ProductLocationStockDialog({ open, onOpenChange, product }) {
  ...
  const { mutate: assignProduct, isLoading } = updateProductLocationStock;
  ...
  const handleAssignToLocation = () => {
    ...
    //  alert("Assigning prodduct ")
    if (!newLocationId || initialStock < 0) {
      setErrorMessage("Please select a valid location and stock quantity.");
      return;
    }
    ...
    assignProduct(
      ...
      {
        onSuccess: () => {
          setMessage("✅ Product successfully assigned to this location!");
          setNewLocationId("");
          setInitialStock(0);
          setReorderLevel(10);
        },
        onError: (err) => {
          setErrorMessage(
            err.message || "❌ Failed to assign product. Try again."
          );
        },
      }
    );
            ...
            {availableLocations.length === 0 ? (
              ...
              <>
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {isLoading ? "Assigning..." : "Assign Product to Location"}
