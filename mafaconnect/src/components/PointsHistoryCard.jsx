...
export function PointsHistoryCard() {
  ...
  // --- MAIN DATA REQUEST ---
  const { data, isLoading } = useLoyaltyHistory({ token, filters });
  ...
  const list = transactions.filter(t =>
    !search ||
    t.reward?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Loyalty Points Activity</CardTitle>
      </CardHeader>

      <CardContent className="space-y-4">

        {/* ───── Summary Headline Boxes ───── */}
        <div className="grid gap-4 md:grid-cols-4">
          <SummaryCard label="Total Spent" value={summary.totalSpent} color="amber" icon={<TrendingDown/>}/>
          <SummaryCard label="Redeemed" value={summary.redeemed} color="green" icon={<TrendingUp/>}/>
          <SummaryCard label="Expired" value={summary.expired} color="red" icon={<Clock/>}/>
          <SummaryCard label="Cancelled" value={summary.cancelled} color="gray" icon={<Clock/>}/>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search reward or code..."
              className="pl-9"/>
          </div>
          ...
          {/* Date Pickers */}
          <DateButton label="From" value={start} onSelect={setStart}/>
          <DateButton label="To" value={end} onSelect={setEnd}/>

          {(status!=="all"||start||end||search) && (
        ...
        <div className="space-y-2 mt-3">
          ...
          {!isLoading && list.length===0 && 
            <p className="text-center text-muted-foreground py-3">No records</p>
          }

          {list.map((t,i)=>(
            <div key={i} className="p-3 border rounded-md flex justify-between items-center hover:bg-accent duration-100">
              <div>
                <Badge status={t.status}>{t.status}</Badge>

                <p className="text-sm">{t.reward} • {t.points}pts</p>
                <p className="text-xs text-muted-foreground">{format(new Date(t.date), "MMM d, yyyy")}</p>
                <p className="text-xs text-muted-foreground">Code: {t.code}</p>
              </div>

              <span className="font-bold text-right">
                {t.status==="used"?"-":""}{t.points}
              </span>
            </div>
          ))}
        </div>
...
function SummaryCard({label,value,color,icon}) {
  return (
    <div className={`p-3 rounded-lg border bg-${color}-500/10 border-${color}-500/30`}>
      <div className="flex items-center gap-2 mb-1">{icon}<span>{label}</span></div>
      <p className="font-bold text-2xl">{(value||0).toLocaleString()}</p>
    </div>
  );
}

function DateButton({label,value,onSelect}) {
  return (
    <Popover>
