...
export function ProductCard({ product }) {
  ...
  return (
    <Card className="group hover:shadow-lg transition-shadow">
      ...
      {/* PRODUCT IMAGE */}
      <Link to={`/products/${product.productid}`}> {/* FIXED 🔥 */}
      ...
      <CardContent className="p-3 sm:p-4">
        <Link to={`/products/${product.productid}`} className="hover:underline"> {/* FIXED 🔥 */}
        ...
        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
          <span className="text-lg font-bold">{formatCurrency(product.sale_price)}</span>

          {isOutOfStock ? (
            <Badge variant="outline" className="text-xs">Out of Stock</Badge>
          ) : isLowStock ? (
            <Badge variant="destructive" className="text-xs">
              Low Stock {isStaff && `(${qty})`}
            </Badge>
          ) : (
            <Badge variant="default" className="text-xs">
              {isStaff ? `In Stock (${qty})` : "Available"} {/* FIXED 🔥 */}
            </Badge>
          )}
        </div>
      </CardContent>

      {/* ADD TO CART — FIX ID HERE TOO */}
